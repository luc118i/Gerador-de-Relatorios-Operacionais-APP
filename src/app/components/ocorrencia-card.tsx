...
export function OcorrenciaCard({ ocorrencia, onClick }: OcorrenciaCardProps) {
  const tempoParada = calcularTempoParada(ocorrencia.horarioInicial, ocorrencia.horarioFinal);
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-semibold text-lg text-gray-900">{ocorrencia.viagem.prefixo}</span>
            <BaseChip base={ocorrencia.motorista1.base} />
          </div>
          <p className="text-sm text-gray-600">Linha {ocorrencia.viagem.linha}</p>
        </div>
        {ocorrencia.evidencias.length > 0 && (
          <div className="flex items-center gap-1 text-gray-600 bg-gray-50 px-2 py-1 rounded">
            <Camera className="w-4 h-4" />
            <span className="text-sm font-medium">{ocorrencia.evidencias.length}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-sm text-gray-700">
          ...
          <span>{ocorrencia.horarioInicial} - {ocorrencia.horarioFinal}</span>
          <span className="text-gray-400">({tempoParada})</span>
        </div>
        
        <p className="text-sm text-gray-600">
          📍 {ocorrencia.localParada}
        </p>
